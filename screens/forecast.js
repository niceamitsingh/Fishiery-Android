...
const source = {
  uri:
    "http://guru.southindia.cloudapp.azure.com/OSF_Telugu_1780731751869209131.wav"
};
...
const alert_feedback = feedback_point => {
  Alert.alert(acknowledgementText);
  var dataSource = request(
    APIConfig.Base_URL + APIConfig.User_Feedback.urlPath,
    {
      method: APIConfig.User_Feedback.method,
      body: JSON.stringify({
        response_id: this.state.selected_site_array.landing_centre_ID,
...
export default class forecast extends React.Component {
  ...
  async apiCallForForecast(currentCity, currentState) {
    ...
    response = await request(
      "http://104.211.204.132/osf/engine/get_osf/",
      data
    );
